(function planningModule() {
  ...
  async function loadJobDetail(id) {
    ...
    `;
    document.getElementById("jobDocs").innerHTML = `<p class="muted">Nog geen documenten gekoppeld.</p>`;
  ...
  function renderGrid() {
    ...
    planningGrid.style.setProperty("--planning-columns", String(Math.max(technicians.length, 1)));
    ...
    technicians.forEach((tech) => {
      ...
      jobs
        ...
        .forEach((job) => {
          ...
          if (job.urgent) block.classList.add("is-urgent");
          if (job.overdue) block.classList.add("is-overdue");
          ...
          `;
          block.addEventListener("click", () => loadJobDetail(job.id));
  ...
  document.querySelectorAll("[data-view-switch]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelectorAll("[data-view-switch]").forEach((item) => {
        item.classList.toggle("is-active", item.dataset.viewSwitch === state.view);
      });
      loadPlanningData();
    });
  });
